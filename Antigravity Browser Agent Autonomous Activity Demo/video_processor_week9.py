"""
Pure Remedy Solutions - Video Processor  (Week 9 Submission)
Video  : Instagram Auto Publishing Workflow, 30-Day Calendar_First.mp4
Handles: silence removal (gentlest, word-safe) -> speed cap (<=90 s) -> Instagram 9:16 beautification
"""
...
INPUT         = r"C:\Users\ks_pa\Downloads\WEEK 9 SUBMISSION\Instagram Auto Publishing Workflow, 30-Day Calendar_First.mp4"
FOLDER        = r"C:\Users\ks_pa\Downloads\WEEK 9 SUBMISSION"
OUTPUT_EDITED = os.path.join(FOLDER, "Instagram_AutoPublish_Edited.mp4")
OUTPUT_IG     = os.path.join(FOLDER, "Instagram_AutoPublish_Instagram.mp4")
...
def create_instagram(src, out):
    ...
    # Top bar: flanking dots at series-label row height, just inside the bracket arms
    dot_y_top = tr1
    ...
    # Top bar: between series label and tagline
    P.append(f"[d6]drawbox=x={alx}:y={(tr1+tr2)//2}:w={alw}:h=1:color=0x4C1D95@0.5:t=fill[al1]")
    # Top bar: between category and decorative row
    ...
    # =========================================================================
    # TEXT LAYERS
    # NOTE: No brand name or website shown in bars per content guidelines.
    #       Bars carry only topic, series, and automation context.
    # =========================================================================

    # --- Top bar ---

    # Series label: bright cyan, small caps style
    P.append(
        f"[al4]drawtext="
        f"text='n8n  MASTERCLASS  SERIES':"
        f"fontfile='{fb}':"
        f"fontsize={sz_sm}:"
        f"fontcolor=0x22D3EE:"
        f"x=(w-text_w)/2:y={tr1-sz_sm//2}:"
        f"shadowcolor=0x0E7490:shadowx=0:shadowy=2"
        f"[tx1]"
    )

    # Main series identity: large, white-lavender, violet glow
    P.append(
        f"[tx1]drawtext="
        f"text='AI AUTOMATION STUDIO':"
        f"fontfile='{fb}':"
        f"fontsize={sz_xl}:"
        f"fontcolor=0xEDE9FE:"
        f"x=(w-text_w)/2:y={tr2-sz_xl//2}:"
    ...
    P.append(
        f"[tx2]drawtext="
        f"text='Automate  *  Schedule  *  Publish':"
        f"fontfile='{fr}':"
        f"fontsize={sz_md}:"
        f"fontcolor=0x67E8F9:"
        f"x=(w-text_w)/2:y={tr3-sz_md//2}:"
        f"shadowcolor=0x0E7490:shadowx=0:shadowy=2"
        f"[tx3]"
    )
    ...
    P.append(
        f"[tx3]drawtext="
        f"text='~ Instagram Growth Automation ~':"
        f"fontfile='{fr}':"
        f"fontsize={sz_sm}:"
        f"fontcolor=0xA78BFA:"
        f"x=(w-text_w)/2:y={tr4-sz_sm//2}"
        f"[tx4]"
    )

    # --- Bottom bar ---

    # Week / series label: bright cyan with shadow
    P.append(
        f"[tx4]drawtext="
        f"text='Week 9  |  n8n Masterclass':"
        f"fontfile='{fb}':"
        f"fontsize={sz_sm}:"
        f"fontcolor=0x22D3EE:"
    ...
    P.append(
        f"[tx5]drawtext="
        f"text='Instagram Auto Publishing':"
        f"fontfile='{fb}':"
        f"fontsize={sz_xl}:"
    ...
    P.append(
        f"[tx6]drawtext="
        f"text='30-Day Content Calendar':"
        f"fontfile='{fb}':"
        f"fontsize={sz_lg}:"
    ...
    # Cohort attribution: muted violet (no brand name)
    P.append(
        f"[tx7]drawtext="
        f"text='AI Architect Cohort  --  Week 9':"
        f"fontfile='{fr}':"
        f"fontsize={sz_xs}:"
...
def main():
    print("=" * 62)
    print("  PURE REMEDY SOLUTIONS - Video Processor  (Week 9)")
    print("  Instagram Auto Publishing Workflow, 30-Day Calendar")
    print("=" * 62)
