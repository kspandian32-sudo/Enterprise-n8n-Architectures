...
SHEET_NODE_MAP = {
    ...
    "📥 Fetch Completed Clients": "Summary",
    "📝 Update CRM → Archived": "Summary"
}
...
for node in wf['nodes']:
    ...
    if name in EMAIL_NODES:
        ...
        elif name in ["📧 Send Kickoff Reminder", "📧 Send Weekly Check-In", "📧 Send Completion Email"]:
            target_to = "={{ $json.clientEmail }}"
    ...
    # 2. Update Google Sheets Nodes
        ...
        node['parameters']['documentId'] = SHEET_ID
        node['credentials'] = {
    ...
    # 3. Update AI/Gemini Nodes
        ...
        # Initialize HTTP Request defaults for Gemini API
        node['type'] = "n8n-nodes-base.httpRequest"
        node['parameters'] = node.get('parameters', {})
        node['parameters']['url'] = "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent"
        node['parameters']['method'] = "POST"
        node['parameters']['authentication'] = "predefinedCredentialType"
        node['parameters']['nodecredentialtype'] = "httpQueryAuth"
    ...
    elif name == "📥 Fetch Completed Clients":
        node['parameters']['sheetName'] = {"mode": "id", "value": GIDS['Summary']}
        node['parameters']['filtersUI'] = {"values": [{"lookupColumn": "Status", "lookupValue": "Completed"}]}

    elif name == "📝 Update CRM → Archived":
        node['parameters']['sheetName'] = {"mode": "id", "value": GIDS['Summary']}
        node['parameters']['filter'] = {"values": [{"column": "clientName", "value": "={{ $json.clientName }}"}]}
        node['parameters']['columns'] = {
            "mappingMode": "defineBelow",
...
# Flow D Rewire: Completion -> Safe Mode -> Email -> Archive
if "📥 Fetch Completed Clients" in conn:
    conn["📥 Fetch Completed Clients"]["main"][0] = [
        {"node": "🤖 Gemini: Write Completion Report", "type": "main", "index": 0}
    ]

conn["🤖 Gemini: Write Completion Report"] = {
    "main": [[{"node": "🛡️ IF: Safe Mode? (Completion)", "type": "main", "index": 0}]]
}

conn["🛡️ IF: Safe Mode? (Completion)"] = {
    "main": [
        [], # True -> Do nothing
        [{"node": "📧 Send Completion Email", "type": "main", "index": 0}] # False -> Send
    ]
}

# Ensure Archive follows Email
conn["📧 Send Completion Email"] = {
    "main": [[{"node": "📝 Update CRM → Archived", "type": "main", "index": 0}]]
}
